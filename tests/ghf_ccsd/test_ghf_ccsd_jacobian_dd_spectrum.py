...
def test_cc_jacobian_spectrum(ghf_ccsd_water_sto3g: GHF_CCSD) -> None:
    ...
    no = ghf_ccsd_water_sto3g.ghf_data.no
    nv = ghf_ccsd_water_sto3g.ghf_data.nv
    dim_s = nv * no
    cc_jacobian = build_cc_jacobian(builders_input)
    cc_jacobian_dd = cc_jacobian[dim_s:, dim_s:]
    dd_eigenvalues = np.linalg.eigvals(cc_jacobian_dd)
